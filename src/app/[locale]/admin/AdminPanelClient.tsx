...
import { canTransitionReservationStatus } from '@/lib/reservations/status';
          ...
          <div className="min-w-0 bg-[var(--background)] p-4 sm:p-6 xl:p-10">
            ...
            {activeTab === 'reservations' && (
              <div className="space-y-5">
                ...
                <div className="space-y-3">{reservations.map((reservation) => <div key={reservation.id} className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className="font-black text-gray-900 dark:text-white">{reservation.id}</span><span className="text-sm text-gray-500">{new Date(reservation.start_at).toLocaleString(locale)}</span></div><p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{reservation.guest_name} · {reservation.guest_phone} · {copy.guests}: {reservation.guests_count}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void updateReservation(reservation.id, 'confirmed')} disabled={!canTransitionReservationStatus(reservation.status as Parameters<typeof canTransitionReservationStatus>[0], 'confirmed')} className="min-h-10 rounded-lg bg-emerald-600 px-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{copy.confirm}</button><button type="button" onClick={() => void updateReservation(reservation.id, 'cancelled')} disabled={!canTransitionReservationStatus(reservation.status as Parameters<typeof canTransitionReservationStatus>[0], 'cancelled')} className="min-h-10 rounded-lg border border-red-200 px-3 text-sm font-bold text-red-700 disabled:cursor-not-allowed disabled:opacity-40">{copy.cancel}</button></div></div>)}{reservations.length === 0 && <p className="rounded-xl bg-gray-50 p-8 text-center text-sm text-gray-500 dark:bg-gray-900/50">{copy.noReservations}</p>}</div>
