...
export async function POST(request: NextRequest) {
  ...
  if (!code || !body.subtotal || !Number.isFinite(Number(body.subtotal)) || Number(body.subtotal) < 0) return NextResponse.json({ error: 'Промокод и сумма обязательны' }, { status: 400 });
