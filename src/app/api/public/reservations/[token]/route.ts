...
export async function GET(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!tenant || !token) return NextResponse.json({ error: apiPublicMessage(request, 'reservationContextTokenRequired') }, { status: 400 });
  const cafe = await prisma.tenants.findFirst({ where: { slug: tenant, status: { in: ['trial', 'active'] } }, select: { id: true } });
  if (!cafe) return NextResponse.json({ error: apiPublicMessage(request, 'cafeNotFound') }, { status: 404 });
  ...
  if (!reservation) return NextResponse.json({ error: apiPublicMessage(request, 'reservationNotFound') }, { status: 404 });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!reservation) return NextResponse.json({ error: apiPublicMessage(request, 'reservationNotFound') }, { status: 404 });
  if (['completed', 'cancelled', 'no_show'].includes(reservation.status)) return NextResponse.json({ error: apiPublicMessage(request, 'reservationCompleted') }, { status: 409 });
