...
export async function POST(request: NextRequest) {
  ...
  if (!body.name?.trim() || !body.phone?.trim() || !/^\d{4}-\d{2}-\d{2}$/.test(body.date || '') || !/^\d{2}:\d{2}$/.test(body.time || '') || !Number.isInteger(guests) || guests < 1 || guests > 50) return NextResponse.json({ error: apiPublicMessage(request, 'waitlistFieldsRequired') }, { status: 400 });
