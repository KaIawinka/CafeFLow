...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!body.id && !body.all) return NextResponse.json({ error: 'Укажите уведомление' }, { status: 400 });
    ...
    if (body.all) {
      ...
      if (updated.count !== 1) return NextResponse.json({ error: 'Уведомление не найдено' }, { status: 404 });
    }
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('User notification update error', error);
    return NextResponse.json({ error: 'Не удалось обновить уведомление' }, { status: 500 });
  }
}
