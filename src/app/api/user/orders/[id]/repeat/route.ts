...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!order?.branch_id) return NextResponse.json({ error: apiUserMessage(request, 'orderBranchUnavailable') }, { status: 404 });
  ...
  if (!items.length) return NextResponse.json({ error: apiUserMessage(request, 'repeatItemsUnavailable') }, { status: 409 });
