...
export async function POST(request: NextRequest) {
  ...
  if (!hostname) return NextResponse.json({ error: 'Некорректное доменное имя' }, { status: 400 });
  ...
  try {
    ...
    if (error && typeof error === 'object' && 'code' in error && error.code === 'P2002') return NextResponse.json({ error: 'Домен уже зарегистрирован' }, { status: 409 });
    return NextResponse.json({ error: 'Не удалось добавить домен' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (!body.id) return NextResponse.json({ error: 'Домен обязателен' }, { status: 400 });
  ...
  if (!domain) return NextResponse.json({ error: 'Домен не найден или уже обработан' }, { status: 404 });
  let records: string[][] = [];
  try { records = await resolveTxt(`_cafeflow.${domain.hostname}`); } catch { return NextResponse.json({ error: 'TXT запись ещё не найдена' }, { status: 409 }); }
  if (!records.some((record) => record.join('').trim() === domain.verification_token)) return NextResponse.json({ error: 'TXT запись не совпадает' }, { status: 409 });
...
export async function DELETE(request: NextRequest) {
  ...
  if (deleted.count !== 1) return NextResponse.json({ error: 'Домен не найден' }, { status: 404 });
