...
export async function POST(request: NextRequest) {
  ...
  if (!userId || !branchId || !validCapabilities(capabilities)) return NextResponse.json({ error: apiAdminMessage(request, 'branchMembershipInputInvalid') }, { status: 400 });
  if (!canGrantCapabilities(auth.role, capabilities)) return NextResponse.json({ error: apiAdminMessage(request, 'capabilitiesGrantDenied') }, { status: 403 });
  if (!canAccessBranch(branchId, auth.branchIds ?? null)) return NextResponse.json({ error: apiAdminMessage(request, 'branchUnavailable') }, { status: 403 });
  ...
  if (!user || !branch) return NextResponse.json({ error: apiAdminMessage(request, 'userOrBranchNotFound') }, { status: 404 });
...
export async function DELETE(request: NextRequest) {
  ...
  if (!membershipId) return NextResponse.json({ error: apiAdminMessage(request, 'membershipIdRequired') }, { status: 400 });
  ...
  if (!membership) return NextResponse.json({ error: apiAdminMessage(request, 'membershipNotFound') }, { status: 404 });
