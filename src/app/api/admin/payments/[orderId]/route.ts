...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  ...
  if (!body.status || !allowed.includes(body.status)) return NextResponse.json({ error: apiAdminMessage(request, 'invalidPaymentStatus') }, { status: 400 });
  if (!auth.tenantId) return apiError(request, 'tenantNotConfigured', 409);
  ...
  if (!payment) return NextResponse.json({ error: apiAdminMessage(request, 'paymentNotFound') }, { status: 404 });
  if (['refunded', 'failed'].includes(payment.status) && body.status === 'paid') return NextResponse.json({ error: apiAdminMessage(request, 'paymentCannotReturnToPaid') }, { status: 409 });
  let amount: Prisma.Decimal;
  try { amount = body.amount ? new Prisma.Decimal(body.amount) : body.status === 'refunded' ? payment.captured_amount.sub(payment.refunded_amount).sub(payment.chargeback_amount) : payment.amount.sub(payment.captured_amount); } catch { return NextResponse.json({ error: apiAdminMessage(request, 'invalidPaymentAmount') }, { status: 400 }); }
  ...
  if (amount.lte(0)) return NextResponse.json({ error: apiAdminMessage(request, 'paymentAmountMustBePositive') }, { status: 400 });
  if ((body.status === 'paid' || body.status === 'partial') && amount.gt(availableToCapture)) return NextResponse.json({ error: apiAdminMessage(request, 'captureExceedsAvailable') }, { status: 409 });
  if (body.status === 'refunded' && amount.gt(availableToRefund)) return NextResponse.json({ error: apiAdminMessage(request, 'refundExceedsCaptured') }, { status: 409 });
