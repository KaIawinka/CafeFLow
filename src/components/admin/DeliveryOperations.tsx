...
function DeliveryCard({ delivery, couriers, locale, onUpdated }: { delivery: Delivery; couriers: Courier[]; locale: Locale; onUpdated: (delivery: Delivery) => void }) {
  const t = getLocaleTranslations(locale).delivery;
    ...
    <article className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 space-y-2">
          <div className="flex flex-wrap items-center gap-2"><span className="font-black text-gray-900 dark:text-white">{delivery.order.order_number}</span><span className="rounded-full bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-600 dark:bg-gray-700 dark:text-gray-300">{t.statusLabels[delivery.status]}</span>{delivery.retry_count > 0 && <span className="text-xs text-amber-700 dark:text-amber-300">{t.attempts}: {delivery.retry_count + 1}</span>}</div>
          ...
          {delivery.events[0] && <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(delivery.events[0].created_at).toLocaleString(locale)} · {t.statusLabels[delivery.events[0].to_status]}{delivery.events[0].reason ? `: ${delivery.events[0].reason}` : ''}</p>}
        </div>
        <div className="grid w-full gap-3 sm:grid-cols-2 lg:max-w-xl">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t.status}<select value={status} onChange={(event) => setStatus(event.target.value as DeliveryStatus)} disabled={isSaving} className="mt-1 min-h-10 w-full rounded-lg border border-gray-300 bg-white px-3 dark:border-gray-600 dark:bg-gray-900 dark:text-white">{transitions[delivery.status].map((value) => <option key={value} value={value}>{t.statusLabels[value]}</option>)}</select></label>
  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-6 dark:bg-gray-900 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-amber-700 dark:text-amber-400">CafeFlow / {t.operations}</p><h1 className="mt-1 text-3xl font-black text-gray-900 dark:text-white">{t.title}</h1><p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{t.subtitle}</p></div><div className="flex gap-2"><select value={status} onChange={(event) => { setIsLoading(true); setStatus(event.target.value as DeliveryStatus | 'all'); }} className="min-h-10 min-w-0 flex-1 rounded-lg border border-gray-300 bg-white px-3 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white"><option value="all">{t.all}</option>{(['pending', 'assigned', 'delivering', 'delivered', 'failed'] as DeliveryStatus[]).map((item) => <option key={item} value={item}>{t.statusLabels[item]}</option>)}</select><button type="button" onClick={() => { setIsLoading(true); void load(); }} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 dark:border-gray-700 dark:text-gray-200" aria-label={t.refresh} title={t.refresh}><RefreshCw className="h-4 w-4" /></button></div></header>
