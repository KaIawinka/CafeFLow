...
export function MenuCarousel({ dishes, locale, viewAll }: { dishes: Dish[]; locale: Locale; viewAll: string }) {
  ...
  const step = 360 / visibleDishes.length;
      ...
      <div className="landing-menu-carousel__viewport">
        <div ref={trackRef} className="landing-menu-carousel__track">
          {visibleDishes.map((dish, index) => (
            <article key={dish.name} onMouseEnter={() => centerCard(index)} className={`landing-menu-carousel__slide ${index === active ? 'is-active' : ''}`} style={{ '--menu-card-angle': `${index * step}deg`, '--menu-card-offset': '0px' } as React.CSSProperties}>
