    ...
    <section className="relative isolate overflow-hidden bg-[#151a1e] text-white">
      ...
      <div className="mx-auto grid min-h-[min(780px,calc(100vh-4rem))] max-w-7xl items-center gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16 lg:px-12 lg:py-24">
        <div className="landing-reveal max-w-2xl">
          <div className="mb-7 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.22em] text-orange-300"><span className="h-px w-10 bg-orange-500" /> CaféFlow · Бишкек</div>
          ...
          <div className="mt-14 grid max-w-lg grid-cols-3 gap-5 border-t border-white/15 pt-5 text-sm">
            <div><p className="font-bold text-white">4.9 / 5</p><p className="mt-1 text-white/50">{t.hero.title === 'Добро пожаловать' ? 'Оценка гостей' : 'Guest rating'}</p></div>
            <div><p className="font-bold text-white">10:00–23:00</p><p className="mt-1 flex items-center gap-1 text-white/50"><Clock3 className="h-3 w-3" /> {t.contact.hours}</p></div>
            <div><p className="font-bold text-white">Bishkek</p><p className="mt-1 flex items-center gap-1 text-white/50"><MapPin className="h-3 w-3" /> {t.contact.address}</p></div>
          </div>
        </div>
