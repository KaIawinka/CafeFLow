      ...
      <section id="menu" className="landing-section landing-section--light bg-[#fffdf8] py-20 text-[#151a1e] sm:py-28">
        <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {localizedDishes.map((dish) => <article key={dish.name} className="group/menu-card flex h-full flex-col overflow-hidden rounded-lg border border-[#ddd8cd] bg-[#f7f5f0] transition-[transform,box-shadow,border-color] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform hover:-translate-y-1 hover:border-[#cfc8ba] hover:shadow-[0_20px_40px_rgba(21,26,30,0.14)] motion-reduce:transition-none motion-reduce:hover:transform-none"><div className="relative h-64 shrink-0 overflow-hidden"><ThemeAwareBackground darkSrc={dish.image} lightSrc={dish.lightImage} className="will-change-transform group-hover/menu-card:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent" /><span className="absolute left-4 top-4 rounded-full bg-orange-500 px-3 py-1 text-xs font-bold text-white">{dish.label}</span></div><div className="flex flex-1 flex-col p-5"><div className="flex min-h-14 items-start justify-between gap-3"><h3 className="text-lg font-bold leading-6">{dish.name}</h3><span className="whitespace-nowrap text-sm font-black text-orange-600">{dish.price}</span></div><p className="mt-2 flex-1 text-sm leading-6 text-[#687078]">{dish.description}</p><Link href={`/${locale}/menu`} className="group/menu-action mt-5 flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-md border border-[#ddd8cd] text-sm font-bold text-[#151a1e] transition-[transform,background-color,border-color,color] duration-300 ease-out hover:-translate-y-0.5 hover:border-orange-500 hover:bg-orange-500/10 hover:text-orange-600">{t.menu.viewAll} <ArrowUpRight className="h-4 w-4 transition-transform duration-300 ease-out group-hover/menu-action:translate-x-1 group-hover/menu-action:-translate-y-0.5" /></Link></div></article>)}
          </div>
        </div>
      </section>
    </>
