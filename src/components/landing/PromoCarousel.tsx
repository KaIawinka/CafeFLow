...
const promoMeta = [
  { href: 'menu', image: 'https://images.unsplash.com/photo-1533089860892-a7c6f0a88666?auto=format&fit=crop&w=1500&q=90' },
  { href: 'menu', image: 'https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=1500&q=90' },
  { href: 'booking', image: 'https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=1500&q=90' },
];
    ...
    <section id="promotions" className="relative z-10 mx-auto -mt-12 w-[calc(100%-2rem)] max-w-6xl scroll-mt-28 sm:-mt-20 sm:w-[calc(100%-3rem)]">
      <div className="relative min-h-[360px] overflow-hidden rounded-[1.5rem] border border-white/15 bg-[#151a1e] text-white shadow-[0_24px_70px_rgba(21,26,30,0.32)] sm:min-h-[390px]">
        ...
        <div className="relative flex min-h-[360px] flex-col justify-between p-7 sm:min-h-[390px] sm:p-10 lg:p-14">
          ...
          <div className="flex items-end justify-end gap-2"><button type="button" onClick={() => move(-1)} aria-label={landing.promotions.previous} className="flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-black/20 transition hover:border-[#f97316] hover:bg-[#f97316]"><ArrowLeft className="h-4 w-4" /></button><div className="flex items-center gap-1.5 px-2">{items.map((promo, index) => <button key={promo.title} type="button" onClick={() => setActive(index)} aria-label={`${landing.promotions.slide} ${index + 1}`} className={`h-1.5 rounded-full transition-all ${active === index ? 'w-10 bg-[#f97316]' : 'w-2 bg-white/35'}`} />)}</div><button type="button" onClick={() => move(1)} aria-label={landing.promotions.next} className="flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-black/20 transition hover:border-[#f97316] hover:bg-[#f97316]"><ArrowRight className="h-4 w-4" /></button></div>
        </div>
      </div>
    </section>
  );
